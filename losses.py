...
__all__ = ['BCEDiceLoss', 'BCELoss', 'FocalLoss', 'MixedLoss', 'BCESoftmaxLoss', 'JaccardLoss']

class JaccardLoss(_Loss):
    def __init__(self, mode : str, classes : Optional[List[int]] =None,
        log_loss : bool = False, from_logits : bool = True, smmoth : float = 0., eps : float = 1e-7):
    #an implementation of the Jaccard Loss for segmentation tasks supporting binary, multiclass, and multilable case
...
class BCESoftmaxLoss(nn.Module):
  ...
  def forward(self, input, target):
    if (len(target.shape) == 4):
      target_ = torch.squeeze(target, 1).long()
      bce = F.cross_entropy(input, target_)
    else:
      bce = F.cross_entropy(input, target)
    smooth = 1e-5
    input = torch.softmax(input, dim = 1)
    input_ = torch.argmax(input, dim = 1, keep_dim = True)
    predict =torch.zeros_like(input_)
    predict = torch.where(input_!=0,  1, predict)
...
def dice_loss(input, target):
    """
    input : (1, H, W) in the range of 0-1
    target : (1, H, W) in 0 or 1
    """
    smooth = 1.0
    input = torch.sigmoid(input)
    iflat = input.view(-1)
